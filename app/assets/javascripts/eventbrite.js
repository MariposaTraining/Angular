...
Eventbrite.prototype = {
    ...
    'request': function(method, params, cb) {
        ...
        if (this.auth_tokens.access_token === undefined) {
            if (this.auth_tokens.app_key) {
                params.app_key = this.auth_tokens.app_key;
            }
            if (this.auth_tokens.user_key) {
                params.user_key = this.auth_tokens.user_key;
            }
            if (this.auth_tokens.user) {
                params.user = this.auth_tokens.user;
            }
            if (this.auth_tokens.password) {
                params.password = this.auth_tokens.password;
            }
        } else {
            auth_headers = {
                'Authorization': 'Bearer ' + this.auth_tokens.access_token
            };
    ...
    'utils': {
        ...
        'eventListRow': function(evnt) {
            ...
            html = "<div class='eb_event_list_item' id='evnt_div_" + evnt.id + "'>" + "<span class='eb_event_list_title'><a href='" + evnt.url + "'>" + evnt.title + "</a></span>" + "<span class='eb_event_list_date'>" + date_string + "</span><span class='eb_event_list_time'>" + time_string + "</span>" + "<span class='eb_event_list_location'>" + venue_name + "</span></div>";
            return html;
        },
    ...
    'widget': {
        'login': function(options, cb) {
            if (window.location.hash.indexOf("token_type=Bearer") !== -1 && window.location.hash.indexOf("access_token=") !== -1 && options.access_token === undefined) {
                if (window.location.hash.slice(window.location.hash.indexOf("access_token=") + 13).indexOf("&") !== -1) {
                    access_token = window.location.hash.slice(window.location.hash.indexOf("access_token=") + 13, window.location.hash.indexOf("access_token=") + 13 + window.location.hash.slice(window.location.hash.indexOf("access_token=") + 13).indexOf("&"));
                } else {
            ...
            if (options.error_message !== undefined && options.error_message == "disabled") {
                ...
                if (window.location.search.slice(window.location.search.indexOf("error=") + 6).indexOf("&") !== -1) {
                    options.error_message = window.location.search.slice(window.location.search.indexOf("error=") + 6, window.location.search.indexOf("error=") + 6 + window.location.search.slice(window.location.search.indexOf("error=") + 6).indexOf("&"));
                } else {
        ...
        'loginHTML': function(strings) {
            ...
            if (strings.user_name !== undefined && strings.user_email !== undefined && strings.logout_link !== undefined) {
